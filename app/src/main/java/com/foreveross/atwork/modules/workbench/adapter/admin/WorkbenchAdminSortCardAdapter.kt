...
class WorkbenchAdminSortCardAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): AbstractDraggableItemViewHolder {
        return when (viewType) {
            ITEM_TYPE_TITLE -> {
                val view = LayoutInflater.from(parent.context).inflate(R.layout.component_workbench_admin_sort_cards_item_title, parent, false)
                AdminSortCardTitleViewHolder(view)
            }

            else -> {
                val view = LayoutInflater.from(parent.context).inflate(R.layout.component_workbench_admin_sort_cards_item, parent, false)
                val customSortCardItemViewHolder = AdminSortCardItemViewHolder(view)
    ...
    override fun onBindViewHolder(holder: AbstractDraggableItemViewHolder, position: Int) {
        ...
        when (holder) {
            is AdminSortCardTitleViewHolder -> {
                holder.tvTitle.text = customSortCardItem.titleContent
            }

            is AdminSortCardItemViewHolder -> {
...
//        }
    ...
    override fun onGetItemDraggableRange(holder: AbstractDraggableItemViewHolder, position: Int): ItemDraggableRange? {
        ...
        return ItemDraggableRange(0, lastPosition)
    ...
    override fun onCheckCanStartDrag(holder: AbstractDraggableItemViewHolder, position: Int, x: Int, y: Int): Boolean {
        ...
        if (holder is AdminSortCardItemViewHolder) {
    ...
    class AdminSortCardTitleViewHolder(itemView: View) : AbstractDraggableItemViewHolder(itemView) {
    ...
    class AdminSortCardItemViewHolder(itemView: View) : AbstractDraggableItemViewHolder(itemView) {
