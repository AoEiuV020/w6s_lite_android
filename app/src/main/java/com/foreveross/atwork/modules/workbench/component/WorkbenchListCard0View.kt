...
class WorkbenchListCard0View : WorkbenchBasicCardView<WorkbenchListTypeCard> {


    private lateinit var adapter: WorkbenchList0Adapter
    private val itemList = arrayListOf<WorkbenchListItem>()

    var skeletonView: WorkbenchList0CardSkeletonView? = null
    ...
    override fun findViews(context: Context) {
        LayoutInflater.from(context).inflate(R.layout.component_workbench_list_card_0, this)

    }

    private fun initViews() {
        adapter = WorkbenchList0Adapter(itemList)
    ...
    override fun skeletonView(): View? {
        ...
        if(null == skeletonView) {
            skeletonView = WorkbenchList0CardSkeletonView(context)
        }
        return skeletonView
    }
    ...
    override fun getViewType(): Int = WorkbenchCardType.LIST_0.hashCode()
