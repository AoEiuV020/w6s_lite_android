...
public class GridPasswordView extends LinearLayout implements PasswordView {
    private static final int DEFAULT_PASSWORDLENGTH = 4;
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            ...
            if (newStr.length() == 1) {
                ...
                for (int i = 0; i < newStr.length(); i++) {
                    ...
                    String newNum = String.valueOf(newStr.charAt(i));
                    for (int j = 0; j < passwordArr.length; j++) {
                        if (passwordArr[j] == null) {
                            passwordArr[j] = newNum;
                            viewArr[j].setText(newNum);
                            notifyTextChanged();
                            break;
                        }
                    }
                    inputView.removeTextChangedListener(this);
                    inputView.setText(passwordArr[i]);
                    inputView.setSelection(1);
                    inputView.addTextChangedListener(this);
                }
            }
