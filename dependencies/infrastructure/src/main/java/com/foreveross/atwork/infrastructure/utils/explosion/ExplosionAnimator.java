...
public class ExplosionAnimator extends ValueAnimator {
    ...
    public ExplosionAnimator(View container, Bitmap bitmap, Rect bound) {
        ...
        for (int i = 0; i < partLen; i++) {
            for (int j = 0; j < partLen; j++) {
                mParticles[(i * partLen) + j] = generateParticle(getRandomColor(), random);
            }
        }
    ...
    private int getBitmapPixelColor(Bitmap bitmap, int w, int h, int i, int j) {
        return bitmap.getPixel((j + 1) * w, (i + 1) * h);
    }
