...
public class DataSchemasDBHelper implements DBHelper {
    ...
    public static final String TABLE_NAME = "schemas_v3_";

    /**
     * create table schemas_ (
     * data_schema_id_ text ,org_code_ text ,
     ...
     * primary key ( data_schema_id_ , org_code_),
    ...
    private static final String SQL_EXEC = CREATE_TABLE + TABLE_NAME + LEFT_BRACKET +
            DBColumn.DATA_SCHEMA_ID + TEXT + COMMA +
            ...
            DBColumn.VISIBLE_RANGE + TEXT + COMMA +
            PRIMARY_KEY + LEFT_BRACKET + DBColumn.DATA_SCHEMA_ID + COMMA + DBColumn.ORG_CODE + RIGHT_BRACKET + COMMA +
            FOREIGN_KEY + LEFT_BRACKET + DBColumn.ORG_CODE + RIGHT_BRACKET + REFERENCES + OrganizationDBHelper.TABLE_NAME + LEFT_BRACKET + OrganizationDBHelper.DBColumn.ORG_CODE + RIGHT_BRACKET +
            RIGHT_BRACKET;
    ...
    public static ContentValues getContentValues(DataSchema dataSchema) {
        ...
        cv.put(DBColumn.VISIBLE_RANGE, dataSchema.mVisibleRange);
        return cv;
    }
    ...
    public static DataSchema fromCursor(Cursor cursor) {
        ...
        if ((index = cursor.getColumnIndex(DBColumn.VISIBLE_RANGE)) != -1) {
            dataSchema.mVisibleRange = cursor.getString(index);
        }
    ...
    public class DBColumn {
        ...
        public static final String VISIBLE_RANGE = "visible_range_";
